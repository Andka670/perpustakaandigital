...
if submit:
    if not user_label or not buku_judul:
        ...
        if selected_book["stok"] <= 0:
            ...
            try:
                # Hitung denda jika tanggal pinjam sudah lewat tanggal kembali
                today = date.today()
                denda = 10000 if today > tanggal_kembali else 0
...
# ----------------------------
# Update denda otomatis untuk semua peminjaman aktif
# ----------------------------
try:
    peminjaman_data = supabase.table("peminjaman").select("*").eq("status", "dipinjam").execute().data
    for p in peminjaman_data:
        t_kembali = datetime.strptime(p["tanggal_kembali"], "%Y-%m-%d").date()
        denda_baru = 10000 if date.today() > t_kembali else 0
        if p.get("denda", 0) != denda_baru:
            supabase.table("peminjaman").update({"denda": denda_baru}).eq("id_peminjaman", p["id_peminjaman"]).execute()
except:
    pass
