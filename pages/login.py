...
@keyframes float {0% { transform: translateY(20px); }50% { transform: translateY(50px); }100% { transform: translateY(20px); }}
...
.login-title { font-weight:bold;color:white;font-size:55px;border-bottom:4px solid white;padding-bottom:5px;text-align:center;}
.animated-subtitle { font-size:18px;margin-top:8px;margin-bottom:30px;font-style:italic;font-weight:bold;color:#f5f5f5; animation:colorchange 4s infinite;}
...
.stButton>button { padding:10px 299px; border-radius:12px; background:linear-gradient(90deg,#6a11cb,#2575fc); color:white; font-weight:bold; font-size:20px; border:none; transition:all 0.3s ease; box-shadow:0 4px 25px rgba(0,0,0,0.35);}
...
.stButton[data-st-key="login_btn"] > button { width:100% !important; height:70px !important;}

/* Animasi buku jatuh */
.book-emoji { position: fixed; font-size:50px; top:-50px; animation: fall linear forwards; z-index:9999; }
@keyframes fall {0%{top:-50px; transform:rotate(0deg);}100%{top:80%; transform:rotate(360deg);}}
</style>
...
<div style="display:flex; align-items:center; justify-content:center; flex-direction:column; gap:10px;">
    <img class="animated-photo" src="https://upload.wikimedia.org/wikipedia/commons/9/99/Sample_User_Icon.png" style="margin-bottom:30px;">
...
# ----------------------------
# Login otomatis + animasi + redirect JS
# ----------------------------
...
# ----------------------------
st.markdown("<div style='text-align:center; font-size:15px; color:black; font-weight:bold'>Belum punya akun?</div>", unsafe_allow_html=True)
