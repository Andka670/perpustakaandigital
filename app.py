...
if "edit" in st.session_state:
    ...
    edit_judul = st.text_input("Judul Baru", value=book_edit.get("judul") or "", key="edit_judul")
    edit_penulis = st.text_input("Penulis Baru", value=book_edit.get("penulis") or "", key="edit_penulis")
    edit_tahun = st.number_input("Tahun Terbit Baru", value=book_edit.get("tahun") or 2000, key="edit_tahun")
    edit_stok = st.number_input("Stok Baru", value=book_edit.get("stok") or 0, key="edit_stok")
    edit_genre = st.selectbox("Genre Baru", 
                              genre_options, 
                              index=genre_options.index(book_edit.get("genre")) if book_edit.get("genre") in genre_options else 0, 
                              key="edit_genre")
    edit_deskripsi = st.text_area("Deskripsi Baru", value=book_edit.get("deskripsi") or "", key="edit_deskripsi")
    ...
    if st.button("Update Detail Buku"):
        ...
        if edit_judul != book_edit.get("judul"):
        ...
        if update_book:
            ...
            supabase.table("buku").update({
                ...
                "cover_url": cover_url,
                "pdf_url": pdf_url
            }).eq("id_buku", book_edit.get("id_buku")).execute()
