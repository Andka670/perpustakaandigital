...
# =====================================================
# Tema CSS Global (Siang/Malam) untuk semua halaman
# =====================================================
if st.session_state.tema == "Siang":
    st.markdown("""
    <style>
    html, body, [data-testid="stAppViewContainer"], .stApp {background: linear-gradient(to bottom, #87ceeb, #ffffff); height: 100%; margin: 0; overflow: hidden;}
    .sun {position: absolute; top: 50px; left: 70%; width: 120px; height: 120px; background: radial-gradient(circle, #FFD700 60%, #FFA500 100%); border-radius: 50%; box-shadow: 0 0 80px 20px rgba(255, 223, 0, 0.7); animation: shine 6s ease-in-out infinite alternate;}
    @keyframes shine {0% { transform: scale(1); opacity: 0.9; } 100% { transform: scale(1.1); opacity: 1; }}
    .cloud {position: absolute; background: radial-gradient(circle at 30% 30%, #fff 70%, #f0f0f0 100%); border-radius: 50%; opacity: 0.9; animation: moveClouds 60s linear infinite;}
    ...
    .moon {position: absolute; top: 60px; left: 70%; width: 100px; height: 100px; background: radial-gradient(circle, #fdfd96 60%, #f4e04d 100%); border-radius: 50%; box-shadow: 0 0 60px 10px rgba(255, 255, 200, 0.6); animation: glow 5s ease-in-out infinite alternate;}
    ...
    with st.form("ubah_password_form"):
        ...
        if submitted:
            if new_pw != confirm_pw:
                st.warning("⚠️ Password baru dan konfirmasi tidak sama.")
            elif not old_pw or not new_pw:
                st.warning("⚠️ Semua field harus diisi.")
            else:
                try:
                    update = supabase.table("user").update({"password": new_pw}).eq("id_user", user["id_user"]).execute()
