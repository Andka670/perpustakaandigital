...
.book-title{font-weight:bold;font-size:15px;margin:8px 0;flex-grow:1;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;text-overflow:ellipsis;min-height:50px;color:brown !important;}
    ...
    if not pinjam_data:
        ...
        for p in pinjam_data:
            ...
            if tgl_kembali:
                ...
                denda = max((today - tgl_kembali_dt).days * denda_per_hari, 0) if p.get("status","").lower() != "sudah dikembalikan" else 0
            else:
                denda = 0
